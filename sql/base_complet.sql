-- ==========================================================
-- FINTECH_VULNERABLE - BASE COMPLÈTE DE DONNÉES
-- Généré pour migration facile sur tout nouvel environnement
-- ==========================================================
...
-- Table: users (Comprend tous les champs pour compatibilité et lab)
...
-- Table: accounts (Utilisée par le Dashboard pour les soldes réels)
...
-- Table: wallets (Table secondaire pour compatibilité historique)
...
-- Table: transactions_v2 (Utilisée par le moniteur global et les transferts)
...
-- 3. INSERTION DES DONNÉES (Identifiants et Soldes)
...
-- Insertion des Comptes (Synchronisés avec les soldes utilisateurs)
...
ALTER TABLE `transactions_v2`
  ADD CONSTRAINT `transactions_v2_ibfk_1` FOREIGN KEY (`from_user_id`) REFERENCES `users` (`id`),
  ADD CONSTRAINT `transactions_v2_ibfk_2` FOREIGN KEY (`to_user_id`) REFERENCES `users` (`id`);
